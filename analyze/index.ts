...
import {UserAgentToTestPickType, IUserAgentToTestPickType} from "@double-agent/profiler/interfaces/IUserAgentToTest";
...
export default class Analyze {
  ...
  public addOverTime(sessionsDir: string, pickType: IUserAgentToTestPickType) {
    ...
    const dirNames = Fs.readdirSync(sessionsDir).filter(x => x.startsWith(pickType)).sort();
  ...
  public generateTestResults() {
    ...
    for (const pickType of pickTypes) {
      ...
      for (const sessionResult of this.resultsMap.byPickType[pickType]) {
        ...
        const humanScoreOverTime = flags.reduce((score, flag) => Math.min(score, flag.humanScore), 0);
      ...
      humanScoreMap.total[pickType] = sessionDetails.map(x => x.humanScore.total).reduce((a,b) => Math.max(a, b), 0);
    }
