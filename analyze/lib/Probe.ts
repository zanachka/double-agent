...
export default class Probe {
  ...
  constructor(id: string, checkName: string, checkType: ICheckType, path: string, args: any[], pluginId: string) {
    this.id = id;
  ...
  public get check() {
    if (!this._check) {
      ...
      let Check: any;
      try {
        Check = require(`${pluginChecksDir}/${this.checkName}`);
      } catch (err) {
        Check = require(`${checksDir}/${this.checkName}`);
      }
      this._check = new Check.default({}, this.path, ...this.args);
    }
    return this._check;
  }
