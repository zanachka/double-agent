...
export default function extractBrowserGroupings(useragentIds: string[]): [string, string[]][] {
  ...
  for (const useragentId of useragentIds) {
    const { osName, osVersion, browserName, browserVersion } = Profiler.extractMetaFromUseragentId(useragentId);
...
function extractGroupedIds(useragentIds: string[]) {
  ...
  const browserGrouping = extractGroupedIdsBy(Profiler.browserNames, useragentIds);
  const osGrouping = extractGroupedIdsBy(Profiler.osNames, useragentIds);
...
function extractGroupedIdsBy(names: string[], useragentIds: string[]) {
  ...
  for (const name of names) {
    idsByGroup[name] = Profiler.findUseragentIdsByName(name);
