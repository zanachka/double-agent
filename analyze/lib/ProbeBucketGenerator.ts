...
export default class ProbeBucketGenerator {
  ...
  constructor(pluginId: string, meta: IProbeBucketMeta) {
    ...
    const groupsById: { [groupId: string]: { useragentIds: string[], checkType: ICheckType, probeIds: string[] } } = {};
    ...
    if (universalProbeIds.size) {
      const universalProbeIdsByCheckType = Array.from(universalProbeIds).reduce((byCheckType, probeId) => {
        const probe = probesById[probeId];
        const checkType = probe.check.type;
    ...
    for (const groupId of Object.keys(groupsById)) {
      ...
      for (const useragentId of group.useragentIds) {
        ...
        for (const probeId of group.probeIds) {
          ...
          groupsById[newGroupId] = groupsById[newGroupId] || { useragentIds: [useragentId], checkType, probeIds: [] };
    ...
    const sortedGroups = Object.values(groupsById).sort((a,b) => {
      if (a.useragentIds.length === 1 && b.useragentIds.length > 1) {
        return 1;
      } else if (a.useragentIds.length > 1 && b.useragentIds.length == 1) {
        return -1;
      }
    ...
    for (const group of sortedGroups) {
      ...
      const probeBucket = ProbeBucket.create(this.layer, probes, group.useragentIds, meta.matcher, meta.scorer);
